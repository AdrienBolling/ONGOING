...
class KnowledgeGrid:
    ...
    def __init__(
        ...
        Args:
            ...
            learning_rate (float): Wright's learning rate LR, must be in (0.5, 1.0).
                Lower values mean faster learning. The Wright learning exponent
                is derived as b = -log(LR)/log(2), giving b in (0, 1). Defaults
                to 0.8 (the "moderate" rate from Jaber et al. 2013), giving
                b ≈ 0.32. Note that `get_knowledge` returns experience**b — a
                dimensionless factor. The user is expected to combine it with
                their own first-item production time T_1 externally
                (e.g. time_per_unit = T_1 / get_knowledge(...)).
            forgetting_time (float): The time to total forgetting, expressed as
                a multiple of T_1 (i.e. forgetting_time = B / T_1). Larger
                values mean slower forgetting. Defaults to 2000.0 (paper's
                typical value when T_1 = 1).
            decay_step (float): The per-call break length applied by
                `decay_knowledge`, expressed as a multiple of T_1
                (i.e. decay_step = τ / T_1). One call to `decay_knowledge`
                advances time by `decay_step` first-item-equivalents. Defaults
                to 1.0.
        ...
        """
        if not (0.5 < learning_rate < 1.0):
            raise ValueError(
                f"learning_rate must be in (0.5, 1.0) so that the Wright "
                f"exponent b = -log(LR)/log(2) lies in (0, 1); got {learning_rate}."
            )
        if forgetting_time <= 0:
            raise ValueError(f"forgetting_time must be > 0; got {forgetting_time}.")
        if decay_step <= 0:
            raise ValueError(f"decay_step must be > 0; got {decay_step}.")
        ...
        self._forgetting_time = forgetting_time
        ...
        # Wright's learning exponent — used both as the knowledge-factor
        # exponent (knowledge = experience**b) and in the LFCM decay formula.
        self.b = -np.log(self._learning_rate) / np.log(2)
    ...
    def transmit_from(self, other: KnowledgeGrid):
        ...
        if self._shape != other._shape:
            raise ValueError(f"Grid shapes must match: {self._shape} vs {other._shape}")
    ...
    def decay_knowledge(self):
        """Apply one step of decay using the Jaber & Bonney LFCM forgetting
        formula (Eqs. 4–6 of Jaber et al. 2013).  Each call advances time
        by `decay_step` first-item-equivalents.

        The formulas are written in T_1-normalised form so T_1 does not
        appear: τ enters only as `decay_step = τ/T_1` and B enters only as
        `forgetting_time = B/T_1`.

        Cells with zero or near-zero experience are left unchanged to avoid
        log(0)/division-by-zero.  Any cells that produce NaN/Inf from
        numerical edge cases are also left unchanged.
        """
        mask = self._grid > 1e-12
        g = self._grid[mask].copy()
        one_minus_b = 1.0 - self.b

        # Eq. (6), with t(u+n) = T_1/(1-b) · (u+n)^(1-b), so
        # B/t(u+n) = forgetting_time · (1-b) / (u+n)^(1-b).
        ratio = self._forgetting_time * one_minus_b / g**one_minus_b
        denom = np.log1p(ratio)
        safe_denom = np.where(np.abs(denom) < 1e-30, 1.0, denom)
        f = self.b * one_minus_b * np.log(g) / safe_denom

        # Eq. (5), with (1-b)/T_1 · τ = (1-b) · decay_step.
        base_s = one_minus_b * self._decay_step + g**one_minus_b
        base_s = np.maximum(base_s, 1e-30)
        s = base_s ** (1.0 / one_minus_b) - g

        # Eq. (4): u_{i+1} = (u+n)^(1+f/b) · (u+n+s)^(-f/b).
        result = g ** (1.0 + f / self.b) * (g + s) ** (-f / self.b)
    ...
    def expertise_clusters(
        self, knowledge_threshold: float | None = None
    ...
    def geodesic_distance(
        ...
        for axis in range(len(self._shape)):
            ...
            idx_src = (
                np.arange(flat_size).reshape(self._shape)[tuple(slices_src)].ravel()
            )
            idx_dst = (
                np.arange(flat_size).reshape(self._shape)[tuple(slices_dst)].ravel()
            )
        ...
        graph = sparse.csr_matrix((weights, (rows, cols)), shape=(flat_size, flat_size))
    ...
    def save(self, path: str | Path):
        ...
        np.savez(
            ...
            forgetting_time=np.array(self._forgetting_time),
            decay_step=np.array(self._decay_step),
    ...
    def load(cls, path: str | Path) -> KnowledgeGrid:
        ...
        grid = cls(
            ...
            forgetting_time=float(data["forgetting_time"])
            if "forgetting_time" in data.files
            else 2000.0,
            decay_step=float(data["decay_step"]) if "decay_step" in data.files else 1.0,
    ...
    def _render_2d(self):
        ...
        knowledge_grid = np.where(self._grid > 0, self._grid**self.b, 0.0)
    ...
    def _render_stats(self) -> dict:
        ...
        stats = {
            ...
            "coverage (%)": float(
                np.count_nonzero(self._grid) / np.prod(self._shape) * 100
            ),
